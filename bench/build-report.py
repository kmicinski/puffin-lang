...
svg_id = 0
def bar_chart(title, subtitle, rows, unit="s", log=False, ratio_mode=False):
    ...
    for glabel, bars in rows:
        ...
        for (key, v, lo, hi, tip) in bars:
            ...
            y += BAR_H + GAP
        out.append(f'<text x="{LEFT-8}" y="{gy + (y-gy)/2 + 4 - GAP/2:.1f}" class="glabel" text-anchor="end">{html.escape(glabel)}</text>')
        y += GROUP_GAP
...
bench_rows, ratio_rows, rss_rows = [], [], []
geo = []
...
geomean = math.exp(sum(geo)/len(geo))
...
for b in R["benchmarks"]:
    ...
    cells = "".join(
        ...
        for k in ROUTES)
    ratio = rr["puffin-arm64"]["median"] / rr["racket"]["median"]
    table_rows.append(f"<tr><th>{b['name']}</th><td class='desc'>{html.escape(b['desc'])}</td>{cells}"
...
{bar_chart("Puffin arm64 ÷ Racket", "log₂ scale centered at parity; left of the line = Puffin faster", ratio_rows, ratio_mode=True)}
...
<h2>Compile time</h2>
{bar_chart("Compiling Puffin programs", "seconds, log scale; hosted = Racket-run reference compiler, puffincc = the self-hosted native compiler", ct_rows, log=True)}
<p class="note">puffincc (a native binary produced from ~3,000 lines of Puffin) compiles fib
