...
#define RD64() (ip += 8, (i64)((u64)code[ip-8] | ((u64)code[ip-7] << 8) | ((u64)code[ip-6] << 16) | ((u64)code[ip-5] << 24) | \
  ...
  for (;;) {
    ...
    // between instructions -- never inside a prim.
    ...
    case OP_BRLT: { PVM_SAFEPOINT(); u16 a = RD16(), b = RD16(); u32 off = RD32(); if (cf->slots[a] <  cf->slots[b]) ip = off; break; }
    case OP_BRLE: { PVM_SAFEPOINT(); u16 a = RD16(), b = RD16(); u32 off = RD32(); if (cf->slots[a] <= cf->slots[b]) ip = off; break; }
    case OP_BRGT: { PVM_SAFEPOINT(); u16 a = RD16(), b = RD16(); u32 off = RD32(); if (cf->slots[a] >  cf->slots[b]) ip = off; break; }
    case OP_BRGE: { PVM_SAFEPOINT(); u16 a = RD16(), b = RD16(); u32 off = RD32(); if (cf->slots[a] >= cf->slots[b]) ip = off; break; }
