...
Internal notes for contributors working on the module system — the
resolver, name mangling, the `.pufi` interface format, and the
separate-compilation build mechanics. The user-facing reference is
docs/MODULES.md; these notes are not part of the public site.
...
`case` datums) and leaves those alone. This is what makes reserved-
word imports an error (the renamer would rewrite `match`'s binding
occurrences) and what makes `set!` to an imported name just work
(it renames to the exporting module's cell).

Both compilers implement resolution: `src/modules.rkt` is the
...
and `puffincc-src/modules.puf` is puffincc's. Both resolvers compute
IDENTICAL module ids (fnv1a-32 over the entry-relative path), so
`racket src/diff-ir.rkt desugar <module-program>` is a meaningful
cross-compiler oracle; puffincc's `--dump-after <pass>` CLI flag
prints the IR after a pass in file mode (a require DAG cannot arrive
on stdin).

Corpus: `src/test-programs/modules-1..6` runs on every route; the
compile-time failure modes live in `src/test-modules.rkt`.

## Type names through the renamer

Type exports need no dedicated machinery in the resolver: the
renamer is uniform over the module (data positions excepted), and a
type annotation is not a data position, so `[s : Shape]` rewrites
through the import map exactly like a reference to `area` does.
Structural heads (`Int Bool Sym Str Void _ -> ->* List Vec Hash Set
Pairof Mut`) are never in any rename map, so they pass through
untouched.

**Demangled diagnostics.** Resolution gives each non-entry module's
top-level names a mangled spelling (`Shape` → `Shape_shapes_826109a6`);
the resolver registers every mangled → source pair in a table
(`src/system.rkt` / `puffincc-src/system.puf`), and both checkers
consult it ONLY when RENDERING a name or type into an error/warning
message — never when comparing (the mangled spellings ARE the type
identities). The same table demangles exhaustiveness warnings, cast
blame labels, and the display element of ADT cast descs (the desc's
constructor TAG list stays mangled, since those are the runtime
identities the check compares). Byte-identical across the two
compilers; `src/test-modules.rkt` pins that.

Corpus: `src/test-programs/modules-typed/` exercises the type-export
story positively on every route (interpreter, bytecode VM, native
arm64, puffincc-on-wasm).

## Separate compilation

`src/separate.rkt` is the build orchestrator, plus sep-mode hooks in
the pipeline and backend; arm64 only. `--module` produces, under
`build-cache/<pathhash>/`:
...
- **Symbol/string literals go through per-module slot tables.** The
  ...
  static roots); a literal is then one load. The entry unit also
  defines the runtime's whole-program table symbols
...
- **Separate mode compiles at ≤ -O1**: the -O2 AAM layer assumes a
  ...
  locally-unused function). Cross-module inlining stays out.
